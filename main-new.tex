...
\begin{prop}\label{prop:hi2}
 ...
 most $d^{3n}$ such that if $ \bm \sigma =
...
$F=(f_1,\hdots,f_p)$ in $\C[X_1,\hdots,X_n]$, we can build a
...
\begin{algorithm}[!h]
  ...
  \{1,2,\hdots,\lceil 4\epsilon^{-1}nd^{3n} \rceil \},\] 
  \[R :=
...
the output returned in Step 7 will contain one point on each connected
...
\begin{align}
    \deg( \Xi)= \sum_{i=1}^{n-p+1} \deg( \Xi_i) \leq nd^{3n}.
\end{align}
...
T &= \{1,2,\hdots,\lceil 4\epsilon^{-1}nd^{3n} \rceil \},\\
