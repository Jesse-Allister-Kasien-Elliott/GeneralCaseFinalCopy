...
\begin{prop}
  For $i=1,\dots,\delta+1$, there exists a non-zero polynomial
  $\D_{i,2}$ in $\C[(\A_{j,k})_{1 \le j,k \le n}]$ of degree at most
...
as \[(f_1(\A\Xb),\hdots,f_p(\A\Xb)) \in \C((\A_{j,k})_{1 \le j,k \le
  n})[X_1,\dots,X_n]^p.\]

\begin{corollary}\label{prop:RadLagPolarV}
  ...
  \end{equation}
  define a radical ideal in $\C((\A_{j,k})_{1 \le j,k \le
...
\begin{proof}
  ...
  \bm u)$ in $\C((\A_{j,k})_{1 \le j,k \le
    n})[X_1,\dots,X_n,L_1,\dots,L_p]$.
\end{proof}
...
satisfies $\bm H_i(3)$. In what follows, we let ${\frkJ}(i,F^\A)$ be
the ideal generated by $F^\A$ and all $p$-minors of $\jac(F^\A,i)$ in
$\C((\A_{j,k})_{1 \le j,k \le n})[X_1,\dots,X_n]$; in particular, the
defining ideal of $W(i,F^\A)$ is $\sqrt{{\frkJ}(i, F^\A)}$.  For $\bm
u$ in $\C^p$, recall also the definition of the Lagrange system
${\frkL}(i, F^\A, \bm u)$ for $F^\A$ given in
Eq.~\eqref{eqdef:frkLgen}.
Our first lemma is a direct application of results from~\cite{EMP}.
\begin{lemma}\label{lem:6.1}
  Let $\bm u$ be in $\C^p$.  For $j=i,\dots,n$, there exists a
  polynomial $Q_j \in \C((\A_{j,k})_{1 \le j,k \le
    n})[X_1,\dots,X_{i-1},X_j]$, monic in $X_j$ and with $Q_{j}$ in
  ${\frkL}(i, F^\A, \bm u)$.
\end{lemma}
\begin{proof}
  Proposition~\ref{prop:projection} gives the inclusion
  $\sqrt{\frak{I}(i,F^\A)} \subset \sqrt{{\frkL}(i, F^\A, \bm u)}$,
  and Corollary~\ref{prop:RadLagPolarV} shows that ${\frkL}(i, F^\A,
  \bm u)$ is a radical ideal, so that we have $\sqrt{\frak{I}(i,F^\A)}
  \subset{\frkL}(i, F^\A, \bm u)$. Thus, it is enough to exhibit
  polynomials $Q_j$ with the required properties and that belong to
  $\sqrt{\frak{I}(i,F^\A)}$.
  
  If $W(i,F^\A)$ is empty, then $\sqrt{{\frkJ}(i, F^\A)}$ is the
  trivial ideal, so we simply take $Q_j=1$ for all $j$.  Otherwise,
  let $(\fp_\ell)_{1 \le \ell \leq L}$ be the prime components of
  $\sqrt{{\frkJ}(i, F^\A)}$.  By assumption, $L \ge 1$ and all $\fp_\ell$ have
  dimension $i-1$. By \cite[Proposition 1]{EMP}, for all $\ell$,
  \[
    \C((\A_{j,k})_{1 \le j,k \le n})[X_1,\dots,X_{i-1}]\rightarrow\C((\A_{j,k})_{1 \le j,k \le n})[X_1,\dots,X_n]/\fp_\ell
  \] 
  is injective and integral. Therefore polynomials
  $q_{\ell,j}\in\C((\A_{j,k})_{1 \le j,k \le n})[X_1,\dots,X_{i-1},X_j]$ exist, all monic in
  $X_j$, with $q_{\ell,j}\in \fp_\ell$ for each $j$ in
  $\{i,\hdots,n\}.$ Thence, \[Q_{j} := \prod_{1 \le \ell\le L}
  q_{\ell,j}\] is monic in $X_j$ and satisfies $ Q_{j} \in
  \sqrt{\frak{I}(i,F^\A)}$, for each $j \in \{i,\hdots,n\}.$ 
\end{proof}
...
polynomial in $\C((\A_{j,k})_{1 \le j,k \le n})[X_1,\dots,X_n]$, we
will let $D \in \C[(\A_{j,k})_{1 \le j,k \le n}]$ be the minimal
common denominator of all its coefficients, and we will write
$\overline P := D P$, so that $\overline P$ is in $\C[(\A_{j,k})_{1
    \le j,k \le n},X_1,\dots,X_n]$.


\begin{proof} \todo{what do we prove?}
  ...
  \begin{itemize}
  \item $K$ is the ideal defined by these generators in the polynomial
    ring $\C[(\A_{j,k})_{1 \le j,k \le n}),X_1,\dots,X_n]$ in $n^2+n$
    indeterminates.
  \item 
  \end{itemize}
  ...
  Let further $Z$ be the zero-set of $K$ in $\C^{n^2 + n}$. Since all
  polynomials in $F^\A$, resp.\ $\jac(F^\A, i)$, have respective
  degrees at most $2d$ in $\C[(\A_{j,k})_{1 \le j,k \le
      n}),X_1,\dots,X_n]$, resp. $2d-1$, Corollary~\ref{coro:degree}
  shows that $Z$ has degree at most $(2d)^{2n}$.
  
  Let further $K'$ be the ideal generated by the polynomials
  $\frkJ(i,F^\A)$, but this time in $\C((\A_{j,k})_{1 \le j,k \le
    n}),X_1,\dots,X_{i-1})[X_i,\dots,X_n]$. Let $P_j$ be the minimal
  of $X_j$ modulo $\sqrt{K'}$. Then, $P_j$ is in $\C((\A_{j,k})_{1 \le
    j,k \le n})[X_1,\dots,X_{i-1},X_j]$ and $\overline {P_j}$ has
  total degree at most $\deg(Z)$.

  By construction, there exists a non-negative integer $m$ such that 
  $P_j^m$ is in $K'$.  bla bla bla
\end{proof}


\begin{lemma} 
  For $j = i,\dots,n$, there exists $P_j$ in
  $\C((\A_{j,k})_{1 \le j,k \le n})[X_1,\dots,X_{i-1},X_j]$, monic in $X_j$, with $P_{j}$ in
  ${\frkL}(i, F^\A, \bm u)$ and $\deg(\bar P_j)\leq (2d)^{2n}.$
\end{lemma} 
\begin{proof}
  If ${\frkL}(i, F^\A, \bm u)$ is the trivial ideal, we take $P_j=1$
  for all $j$ and we are done. Otherwise, as we pointed out in the
  proof of the previous lemma, this ideal is radical, with all its
  prime components of dimension $i-1$.

  As a result, the existence of the polynomials $Q_j$ established
  above implies that all prime components of ${\frkL}(i, F^\A, \bm u)$
  intersect $\C((\A_{j,k})_{1 \le j,k \le n})[X_1,\dots,X_{i-1}]$
  trivially; in particular, ${\frkL}(i, F^\A, \bm u) \cap
  \C(\A)[X_1,\dots,X_{i-1}] = \{ 0\}$. Thus, if we denote by $\frak K$
  the extension of ${\frkL}(i, F^\A, \bm u) \cap
  \C(\A)[X_1,\dots,X_n]$ given by
  \[
  \frak K :=({\frkL}(i, F^\A, \bm u) \cap \C(\A)[X_1,\dots,X_n])\cdot \C(\frak A, X_1,\dots,X_{i-1})[X_i,\dots,X_n],
  \]
  Thence,
  \begin{equation}\label{eq:3}
    \C(\frak A, X_1,\dots,X_{i-1}) \to \C(\frak A, X_1,\dots,X_{i-1})[X_i,\dots,X_n]/\frak K
  \end{equation}
  is an injective ring extension, and $\C(\frak A,
  X_1,\dots,X_{i-1})[X_i,\dots,X_n]/\frak K$ is a finite-dimensional
  vector space over $\C(\frak A, X_1,\dots,X_{i-1})$. For
  $j=i,\dots,n$, we can then define the minimal polynomial $P_j$ of
  (the multiplication by) $X_j$ in the extension~\eqref{eq:3}; this is
  a monic polynomial in $\C(\A)(X_1,\dots,X_{i-1})[X_j]$.  Using the
  existence of $Q_j$ monic in $X_j$, we now prove that $P_j$ is
  polynomial in $X_1,\dots,X_{i-1}$; we will conclude the proof using
  known degree bounds on $P_j$.

  The polynomial $Q_j$ also belongs to $\frak K$, so that $P_j$
  divides $Q_j$ in $\C(\A,X_1,\dots,X_{i-1})[X_j].$ We can therefore
  write
  \begin{align*}
    Q_j &= P_jR_j,~~~~ P_j,R_j \in \C(\A,X_1,\dots,X_{i-1})[X_j].
  \end{align*}
  It then follows by Gauss's lemma that we can write
  \begin{align*}
    Q_j = p_jr_j, ~~~~p_j,r_j \in \C(\A)[X_1,\dots,X_{i-1},X_j],
  \end{align*}
  such that $\mu_j \in \C(\A,X_1,\dots,X_{i-1})$ exists with 
  \[
  ...
  hence \[P_j=1\cdot p_j=p_j \in \C(\A)[X_1,\dots,X_{i-1},X_j].\] It
  follows that $P_j$ belongs to $\frak K \cap
  \C(\A)[X_1,\dots,X_{i-1},X_j]$. Now, we pointed out that 
  
  Now, consider the polynomials  ${\frkL}(i, F^\A, \bm u)$, 
  \[
  F^\A,\ [L_1~\cdots~L_p]\cdot \jac(F^\A, i),\ u_1 L_1 + \cdots + u_p L_p -1
  \]
  in the polynomial ring in $n^2+n+p$ variables
  $\C[\A,X_1,\dots,X_n,L_1,\dots,L_p]$, let $\frak W$ be their
  zero-set, and let $\deg(\frak W)$ be its degree, in the sense
  of~\cite{H}. Then, Proposition~1 in~\cite{CGR} implies that $P_j$
  has degree at most $\deg(\frak W)$. Since all polynomials defining
  $\frak W$, seen in $\C[\A,X_1,\dots,X_n,L_1,\dots,L_p]$, have degree
  at most $2d$, the B\'ezout inequality of~\cite{H} gives
  \[\deg(P_j) \le (2d)^{n+p-i+1} \le (2d)^{2n}.\qedhere\]
\end{proof}
...
Now we apply the Nullstellensatz for $\pjb$ with the ideal membership
for $\IilfA.$ Let $T$ be a new variable; applying the Nullstellensatz
in $\C(\A)[\Xb,\Lb][T]$, and clearing denominators, we obtain the
existence of $\alpha_j$ in $\C[\A]-\{0\}$ and $ C_{j,\ell},B_j$ in
