...
\begin{prop}\label{prop:Hi123}
  For $i=1,\dots,\delta+1$, there exists a non-zero 
...
\section{Genericity of $\textbf{H}_i^{'}$}\label{Sec:Hip}
...
assumptions $\bm H_i$, and we prove the following.

\begin{prop}\label{prop:hi2}
  Under the assumptions above, for $i=1,\dots,\delta+1$, there exists
  a non-zero polynomial
  \[  \Xi_{i} \in \C[S_1,\dots,S_{i-1}] \]
  of degree at most $d^{3n}$ such that if $ \bm \sigma =
  (\sigma_1,\hdots,\sigma_{i-1}) \in \C^{i-1}$ does not cancel
  $\Xi_{i}$, then for any $\bm u=(u_1,\dots,u_p)$ in $\C^p$,
 $0$ is a regular value of the $n+p$ polynomials in $\C[X_1,\dots,X_n,L_1,\dots,L_p]$
  \begin{equation*}
    X_1 - \sigma_1, \dots, X_{i-1} - \sigma_{i-1},\ F,\ [L_1~\cdots~L_p]\cdot \jac(F, i),\ u_1 L_1 + \cdots + u_p L_p -1.
  \end{equation*}
\end{prop}
The proof occupies the rest of this section; as usual, the index $i$
is fixed throughout.  Let $\Psi: \C^{n+p} \times \C^{i-1} \rightarrow
\C^{n}$ be the mapping defined by the $n+p-1$ polynomials
\[
X_1 - S_1, \dots, X_{i-1} - S_{i-1},\ F,\ [L_1~\cdots~L_p]\cdot \jac(F, i)
\]
in indeterminates $X_1,\dots,X_n,L_1,\hdots,L_p,S_1,\dots,S_{i-1}$.
\begin{lemma}
  $0$ is a regular value of $\Psi$ in the open set $\Omega \times
  \C^{i-1} \subset \C^{n+p} \times \C^{i-1}$, here $\Omega \subset
  \C^{n+p}$ is defined by $(L_1,\dots,L_p) \ne (0,\dots,0)$.
\end{lemma}
\begin{proof}
  We claim that at all zeros $(\xb,\bm \ell,\bm \sigma)$ of $\Psi$,
  with $\bm \ell$ non-zero, the Jacobian matrix of $\Psi$ has full
  rank $n+p-1$ at $(\xb,\bm \ell,\bm \sigma)$. Indeed, indexing columns by
  \[
  ...
  \left[ 
    ...
    \right].
  \]
  Because $\ell$ is non-zero, $\bm H_i(2)$ shows that the Jacobian
...
$d^{(n+p)+(n)}\leq d^{3n},$ with the property that, if $\Xi_{i}(\bm
...
has full rank $n+p-1$ at $(\xb,\bm\ell)$.

We can now finish the proof of Proposition~\ref{prop:hi2}, along the
lines of the proof of Lemma~\ref{prop:RadLagPolarV}. Suppose that
...
Eq.~\eqref{eqdef:fiber2} has full rank $n+p$ at $(\bm x,\bm \ell)$, as
claimed.
