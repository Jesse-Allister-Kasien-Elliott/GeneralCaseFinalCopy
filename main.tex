...
\def\lagFAl{{\bm\lag}(F^{\mA},i,(l_1,\hdots,l_c))}
...
%
\noindent
Let $\lb_Z$ be the vector of rational functions corresponding to the irreducible component $Z$ and consider the open set
\[
\mathscr{O}_{Z} = 
...
%
%
\subsubsection{Reltionaships among determinental and Lagrangian modeling of polar varieties}
...
\item For any $(\xb,\lb)$ in $\mathscr{W}(\pi_i,V(F))$, the Jacobian matrix of $\mathscr{I}(i,F)$ has full rank $p+n-i$ at $(\xb,\lb)$.  {\em By the Jacobian Criterion~\cite[Corollary 16.20]{ECA}, this
...
\[
 X_1-\sigma_1,\dots,X_{i-1}-\sigma_{i-1},F(\xb), \Lag(F,i,(L_1,\hdots,L_p)
\]
...
\noindent 
Finally, assuming that $Z_i$ are the irreducible components of $W(\pi_i,V(F))$ so that
\[
W(\pi_i,V(F)) = \bigcup_{i=1}^r Z_i,
\]
when both $\textbf{H}_i$ and $\textbf{H}_i^{'}$ hold, we in addition say that $\bm u$ satisfies $\textbf{H}_i^{''}$ when 
\begin{enumerate}
...
\begin{proof}
  ...
  Now, let $\bm V = (V_1,\hdots,V_p)$ be new indeterminants and consider the polynomials
  \[
\IilfAV := \left(F^{\A}, \lagFfA,\Vdl-1 \right)
\]
in $\C[\A,\Xb,\Lb,\bm V]$, let $\frak W$ be their zero-set, and let $\deg(\frak W)$ be its
