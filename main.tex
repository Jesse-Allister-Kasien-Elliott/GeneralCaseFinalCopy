...
\begin{theorem}\label{theo:main}
  Let $F= (f_1,\hdots,f_p)\in\ZZ[X_1,\hdots,X_n]^p$ be a sequence of polynomials with $\deg(f_i) \leq d$ and $\htt(f_i) \leq b$. Suppose that the ideal $\langle F \rangle \subset \C[X_1,\hdots,X_n]$ is radical and that $V=V(F) \subset \C^n$ is 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Proof of weak transversality: Proposition \ref{prop:weak_t}}
...
%
\noindent
It follows that when $V=V(F)=V(f)$ is a hypersurface, then the polar variety $W(\pi_i,V(f))$ is defined by  
\[
...
\]
In Section \ref{sec:additionalStatementsLagSysms}, we prove Proposition \ref{prop:polarVs2}, which tells us that for any $\ub \in \C^p$, assuming that $F$ satisfies $\bm H_i(1)$ and $\bm H_i(2)$, we have the inclusion 
    \[
...
%
%
\subsection{Establishing correctness for Lagrange systems}
%
...
%
\begin{proof}
Recall that for $\xb \in Z,$ the rank of $\ji$ is at most $p-1$, because $\Wi$ is defined by the vanishing of $F$ and $\minors$. Now,  at $\xb \in Z,$ consider the Jacobian matrix $\jac_{\xb}(F,i)$  with entries taken modulo the ideal $I(Z)$. Consider these entries as elements in $\C(Z)$ and note that since $Z$ is irreducible, the function field $\C(Z)$ is well defined; let $\jt$ denote this matrix.  Since the $p$-minors of $\ji$ are zero over $\xb \in Z,$ the $p$-minors of $\jt$ are also zero. And therefore the rank of $\jt$ is also at most $p-1$. Thus, some well defined null space element 
\[\lb = (l_{1},\hdots,l_p)=\left(\frac{N_1}{D_1},\hdots,\frac{N_p}{D_p}\right)\in \C(Z)^p-\{\bz\},\]
\noindent 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Genericity statements}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
For each $\xb$ that is a solution of $\eqref{sys:lag},$ there exists an irreducible component $Z$ with $\xb \in Z$ and $\lb_Z(\xb)$ defined and not zero. Let $\upsilon_{\xb}(\ub)$ denote $\ub\cdot \lb_{Z}(\xb)$ and put
\[
\Upsilon_i(\ub) := \prod_{\xb \textrm{ a solution of \eqref{sys:minors}}} \upsilon_{\xb}(\ub).
\]
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The algorithm: Computing at least one point in each connected component of $V(F) \cap \R^n$}
%
...
\end{prop}
\begin{proof}
Since $\xb$ is a solution of $\eqref{sys:minors}$, there exists an irreducible component $Z$ of $W(\pi_i,V)$ with $\xb \in Z$. Then, by $\bm H_i^{'}(2),$ the rational function $\lb_Z(\xb)$ is well defined and not zero. Now, by the same argument given in the proof of Lemma \ref{lem:3.9}, we have that $\xb \in \pi_{\Xb} ( \Wil)$ and $\xb$ is a solution of $\eqref{sys:lag}.$  
\end{proof}
\noindent 
Therefore, by computing the solutions of the equations \eqref{eq:syst2} and then computing the projection on the $\Xb$-space, we will not miss any points on the polar varieties $W(\pi_i,V).$ 
\par 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Proof of $\textbf{H}_i$}\label{sec:applications}
...
%\J_i(\mA \xb,\bm b)
    ...
    %
    \noindent
    Therefore, the polar variety $W(\pi_i,V(F^{\mA}))=\dim Y(\bm b)$ is either empty or ($i-1)$-equidimensional; hence, $\bm H_i(1)$ is established. 
    %
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Proof of the Noether position property: $\textbf{H}_i(3)$}\label{ssec:Hi2}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Proof of $\textbf{H}_i^{'}(1)$}\label{proof:hip}
...
We assume that $F$ satisfies $\textbf{H}_i$, and we prove the following: {\em
  ...
  where the irreducible decomposition of the polar variety is $W(\pi_i,V) = Z_{i,1} \cup Z_{i,2} \cup \hdots \cup Z_{i,r_i}$ and $K_{Z_{i,k}}$ are the polynomials from \eqref{eq:K}.}
