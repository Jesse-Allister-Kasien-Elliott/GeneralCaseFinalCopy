...
classes~\cite{Piene78,Teissier88}); they play a central role in the algorithm, which computes suitable sections of the
...
%
When $n-i \sim 2p,$ 
then the number of minors in the determinantal modeling of the polar varieties: \[S_i = \binom{n-i}{p}\sim 2^{n-i+1}\] and consequentially after applying B\'ezout’s bound to obtain degree estimates, the bound grows exponentially and is thus too large. To avoid this problem and obtain better degree estimates, we will rely on a Lagrangean reformulation of polar varieties. Let $\Lb = (L_1,\hdots,L_p)$ be new indeterminants and consider the ``Lagrange polynomials'' $\mathscr{L}_1,\dots,\mathscr{L}_{n-i}$, with
\[
[\mathscr{L}_1, \hdots, \mathscr{L}_{n-i}]=[L_1,\hdots,L_p]\cdot \jac (F,i),
\] 
in the variables $\Xb=(X_1,\hdots,X_n).$
...
\[
V\left(F, [L_1,\hdots,L_p] \cdot \jac (F,i)\right)
=
...
Recall that $V=V(F)$ is equidimensional with $F = (f_1,\hdots,f_p)$ a sequence of polynomials in  $\C[X_1,\hdots,X_n]$. And recall that we suppose the ideal $\langle f_1,\hdots, f_p \rangle $ is radical and that $V$ is smooth with $\dim V = n-p$. To prove Theorem~\ref{theo:main}, we need quantitative
version of facts we stated in the introcution, namely that in generic coordinates,
...
%
\begin{proof}
Recall that for $\xb \in Z,$ the rank of $\ji$ is at most $p-1$, because $W$ is defined by the vanishing of $F$ and $\minors$. Note that since $Z$ is irreducible, the function field $\C(Z)$ is well defined. Consider $\jac_{\xb}(F,i)$  with entries taken modulo the ideal $I(Z)$, and consider these entries as elements evaluated over $\C(Z);$ let $\jt$ denote this matrix. Since the $p$-minors of $\ji$ are zero over $\xb \in Z,$ the $p$-minors of $\jt$ are zero over $\bm y \in \C(Z).$ And therefore the rank of $\jt$ is also at most $p-1$ at $\bm y \in \C(Z).$ Thus, $\lb_{\xb} \in \C(Z)^p-\{\bz\}$ exists with having rational entries in $\xb$ and with $\lb_{\xb} \cdot \jt = \bz$. And therefore with $\lb_{\xb} \cdot \ji = \bz$ as well.
\end{proof}
%
\noindent
...
\]
Put $D_{\xb} := \prod_{1\leq i \leq p}D_{\xb,i},$ notice that $D_{\xb} \not = \bz$, and put  
\[
...
\subsection{Proof of $\textbf{H}_i(2)$}
   ...
   \J_i(\xb,\bm b)
    ...
    \jac_{\xb}(\pi_i)
     \ebm.
    \] 
    Now, let $L_1, \hdots , L_c$ and $T_1, \hdots , T_d$ be new variables, and notice that the Jacobian
    \[
    ...
    \]
    has full rank $p + n.$  This Jacobian is the following matrix:
    \[
    \left[ 
    \begin{array}{c|c|c}
